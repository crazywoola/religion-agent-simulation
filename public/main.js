...
const guideBookCache = new Map();
let currentGuideBook = null;

async function fetchGuideBook(locale) {
  if (guideBookCache.has(locale)) {
    return guideBookCache.get(locale);
  }
  const res = await fetch(`/api/guide?locale=${encodeURIComponent(locale)}`);
  const data = await res.json();
  guideBookCache.set(locale, data);
  return data;
}
...
async function openGuideBook(chapterIndex = 0) {
  ...
  syncBodyLock();
  currentGuideBook = await fetchGuideBook(i18n.locale);
  renderGuideBook();
}
...
function applyStaticI18n() {
  ...
  if (guideModalEl && !guideModalEl.hidden) {
    openGuideBook(guideChapterIndex);
  }
  renderGameLabTabs();
  renderIntelDropdown();
