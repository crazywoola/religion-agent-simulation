...
ブラウザベースの戦略シミュレーションゲームです。**14の宗教**（世俗運動を含む）が**7つの世界地域**で信徒を巡って競い合います。各宗教は独自の特性、統治ルール、パッシブ能力を持ち、社会の風景を繊細に形作ります。プレイヤーは特定の宗教を操作するのではなく、イベント決定、戦略カード、シグナル調整、リスクベットを通じて世界に影響を与えます。各ランの開始時に**59種のロールアーキタイプ**から宗教と組み合わせたキャラクターを選び、**20種の戦略デッキプリセット**（または108枚のカードプールからランダム抽選）を選択し、固有の勝利目標を追います。
...
- **14の宗教** — 仏教、ヒンドゥー教、道教、イスラム教、プロテスタント、空飛ぶスパゲッティ・モンスター教、カトリック、神道、世俗、ユダヤ教、シク教、東方正教会、ゾロアスター教、バハイ教 — それぞれ独自のメトリクス、特性、統治機構、教義、パッシブ能力を保有
...
- **キャラクター＆ロールシステム** — 59種のロールアーキタイプ（外交官、戦争領主、スパイ、神秘家など）を宗教と組み合わせ；各キャラクターに固有の勝利目標、スターターIntelボーナス、優先カードプールあり
- **108枚の戦略カードプール** — デッキA（外交・安定、54枚）とデッキB（拡張・混乱、54枚）に分割；各ゲームで20種のキュレーテッドプリセットまたはランダム36枚から15枚を選択；即時・条件付き（閾値で倍増）・持続（複数ラウンド効果）の3タイプ
...
- **メタ進行** — 研究点、解放可能なパーク、実績システム、キャラクターとデッキ選択を記録するデイリーチャレンジリーダーボード
- **3D可視化** — Three.jsインタラクティブマップ、アニメーション付きアントライン転化回廊、地域ノード、イベントオーバーレイ
- **豊富なログシステム** — 布教、審判、イベント、パッシブ、領域（キャラクター選択・デッキ選択含む）のログ、文脈対応テキストとフィルター機能
